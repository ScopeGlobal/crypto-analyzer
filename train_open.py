...
def real_time_test(currency):
    ...
    real_time_result = real_time_df["Open"]

    if not os.path.isfile('real_time_open_result_rf'):
        real_time_result.to_csv('real_time_open_result_rf.csv')

    load_model = pickle.load(open(filename, 'rb'))
    result = load_model.predict(cleaned_real_time_df)
    print(result)

    if not os.path.isfile('predicted_open_result_rf'):
        predicted_df = pd.DataFrame(result)
        predicted_df.to_csv('predicted_open_result_rf.csv')
